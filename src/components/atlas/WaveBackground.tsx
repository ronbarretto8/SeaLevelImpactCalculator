 ...
 * Calibrated for ultra-subtle, atmospheric rendering in both Dark and Light modes.
 */
export const WaveBackground = memo(({ variant = "hero", className = "", isDark = true }: WaveBackgroundProps) => {
  // Global opacity reduction for both modes
  const opacity = isDark
    ? variant === "hero"
      ? 0.22
      : 0.16
    : variant === "hero"
      ? 0.4    // Heavily reduced light mode opacity
      : 0.25;  // Heavily reduced light mode opacity
  ...
  // Light mode specific styles - desaturate to avoid bright cyan
  const lightStyle = !isDark ? { mixBlendMode: "multiply" as const, filter: "saturate(0.3) opacity(0.7)" } : {};
  ...
  return (
    ...
    >
      ...
      {isDark ? (
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-white via-transparent to-transparent z-10 pointer-events-none opacity-50" />
      )}
            ...
            <>
              <defs>
                <linearGradient id={`wv-sky-${idSuffix}`} x1="0" x2="0" y1="0" y2="1">
                  <stop offset="0%" stopColor="hsl(215, 15%, 90%)" stopOpacity="0.3" />
                  <stop offset="100%" stopColor="hsl(215, 20%, 85%)" stopOpacity="0.5" />
                </linearGradient>
              </defs>
            ...
            <>
              <defs>
                <linearGradient id={`wv-back-${idSuffix}`} x1="0" x2="0" y1="0" y2="1">
                  <stop offset="0%" stopColor="hsl(215, 20%, 85%)" stopOpacity="0.4" />
                  <stop offset="100%" stopColor="hsl(215, 25%, 80%)" stopOpacity="0.6" />
                </linearGradient>
              </defs>
            ...
            <>
              <defs>
                <linearGradient id={`wv-azure-${idSuffix}`} x1="0" x2="0" y1="0" y2="1">
                  <stop offset="0%" stopColor="hsl(215, 25%, 80%)" stopOpacity="0.5" />
                  <stop offset="100%" stopColor="hsl(215, 30%, 75%)" stopOpacity="0.7" />
                </linearGradient>
              </defs>
            ...
            <>
              <defs>
                <linearGradient id={`wv-mid-${idSuffix}`} x1="0" x2="0" y1="0" y2="1">
                  <stop offset="0%" stopColor="hsl(215, 30%, 75%)" stopOpacity="0.6" />
                  <stop offset="100%" stopColor="hsl(215, 35%, 70%)" stopOpacity="0.8" />
                </linearGradient>
              </defs>
            ...
            <>
              <defs>
                <linearGradient id={`wv-front-${idSuffix}`} x1="0" x2="0" y1="0" y2="1">
                  <stop offset="0%" stopColor="hsl(215, 35%, 70%)" stopOpacity="0.7" />
                  <stop offset="100%" stopColor="hsl(215, 40%, 65%)" stopOpacity="0.9" />
                </linearGradient>
              </defs>
            ...
            <>
              <defs>
                <linearGradient id={`wv-navy-${idSuffix}`} x1="0" x2="0" y1="0" y2="1">
                  <stop offset="0%" stopColor="hsl(215, 40%, 65%)" stopOpacity="0.8" />
                  <stop offset="100%" stopColor="hsl(215, 45%, 60%)" stopOpacity="1" />
                </linearGradient>
              </defs>
