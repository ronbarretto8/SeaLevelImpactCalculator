 ...
 * Calibrated for a cinematic "Deep Sea at Night" aesthetic in Dark Mode.
 */
export const WaveBackground = memo(({ variant = "hero", className = "", isDark = true }: WaveBackgroundProps) => {
  // Global opacity reduction to keep it subtle
  ...
  // DARK THEME: Rich, deep cinematic ocean colors (Navy to dark Teal)
  const darkColors = {
    sky: "#020812",    // Deepest navy night
    back: "#041221",   // Dark ocean depth
    azure: "#061C30",  // Muted deep blue
    mid: "#082942",    // Submarine teal-blue
    front: "#0B3857",  // Atmospheric deep ocean
    navy: "#01050A"    // Near-black crest
  };
  ...
  return (
    <div
      ...
      style={{ opacity }}
    >
      {/* Top Fade Overlay for smooth blending into background */}
      {isDark ? (
        <div className="absolute inset-0 bg-gradient-to-b from-[#020812] via-transparent to-transparent z-10 pointer-events-none opacity-80" />
      ) : (
      ...
      <div className={`relative ${waveHeight} w-full`}>
        ...
        >
          {isDark ? (
            <path fill={darkColors.sky} fillOpacity="0.2" d="M0,170 C200,110 400,230 600,170 C800,110 1000,230 1200,170 C1400,110 1600,230 1800,170 C2000,110 2200,230 2400,170 C2600,110 2800,230 2880,170 L2880,320 L0,320 Z" />
          ) : (
            <>
              <defs>
        ...
        >
          {isDark ? (
            <path fill={darkColors.back} fillOpacity="0.3" d="M0,140 C240,80 480,200 720,140 C960,80 1200,200 1440,140 C1680,80 1920,200 2160,140 C2400,80 2640,200 2880,140 L2880,280 L0,280 Z" />
          ) : (
            <>
              <defs>
        ...
        >
          {isDark ? (
            <path fill={darkColors.azure} fillOpacity="0.4" d="M0,130 C220,70 440,190 660,130 C880,70 1100,190 1320,130 C1540,70 1760,190 1980,130 C2200,70 2420,190 2640,130 C2750,110 2880,150 2880,150 L2880,240 L0,240 Z" />
          ) : (
            <>
              <defs>
        ...
        >
          {isDark ? (
            <path fill={darkColors.mid} fillOpacity="0.5" d="M0,120 C300,60 600,180 900,120 C1200,60 1500,180 1800,120 C2100,60 2400,180 2700,120 C2790,100 2880,140 2880,140 L2880,200 L0,200 Z" />
          ) : (
            <>
              <defs>
        ...
        >
          {isDark ? (
            <path fill={darkColors.front} fillOpacity="0.6" d="M0,90 C240,40 480,140 720,90 C960,40 1200,140 1440,90 C1680,40 1920,140 2160,90 C2400,40 2640,140 2880,90 L2880,150 L0,150 Z" />
          ) : (
            <>
              <defs>
        ...
        >
          {isDark ? (
            <path fill={darkColors.navy} fillOpacity="0.8" d="M0,40 C180,15 360,65 540,40 C720,15 900,65 1080,40 C1260,15 1440,65 1620,40 C1800,15 1980,65 2160,40 C2340,15 2520,65 2700,40 C2790,28 2880,52 2880,52 L2880,80 L0,80 Z" />
          ) : (
            <>
              <defs>
